...
class resend_failed_task extends \core\task\scheduled_task {
  ...
  private function extract_events($limitfrom, $limitnum) {
    ...
    $params = [
      'failedeventstimeoutperiod' => get_config('logstore_xapi', 'failedeventstimeoutperiod'),
    ];
    $select = " timecreated >= " . time() . " - :failedeventstimeoutperiod ";
    $sort = '';
    $fields = '*';
  ...
  private function store_failed_events($events) {
    global $DB;
    $DB->insert_records('logstore_xapi_failed_log', $events);
  }
  ...
  public function execute() {
    if (false === get_config('logstore_xapi', 'resendfailedbatches')) {
      return;
    }
    ...
    $limitfrom = 0;
    while ($extractedevents = $this->extract_events($limitfrom, $store->get_max_batch_size())) {
